...
def get_id_from_match_data(match_data):
    return match_data.ID


def extract_round_names(rounds):
    names = []
    for entry in rounds:
...
def scan_matches(bracket_data, round_names):
    ladder_tree = []
    ...
    for i in range(len(round_names)):
        ...
        ladder_tree[i].sort(key=get_id_from_match_data)
...
class Class(object):
    ...
    def get_match_from_identifier(self, challonge_match_id: int):
    ...
    def get_match(self, challonge_match_id: int, is_identifier: bool = False):
        if is_identifier:
